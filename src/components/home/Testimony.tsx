...
const Testimony = () => {
  ...
  return (
    <section className='my-3 border-b border-black'>
        <CustomText text='Testimonials' textType='medium' weightType='bold' extraStyle='text-center'/>
        <CustomText text="Don't Take Our Word for it" textType='large' weightType='semibold' extraStyle='text-center'/>
        <CustomText text='See what our clients have to say about us.' textType='normal' weightType='medium' color='text-gray-600' extraStyle='text-center mb-3'/>
        ...
        <div className='relative my-8'>
          ...
          { testmonyItems[imageIndex + 1]?.img && (  <div onClick={() => setImageIndex((curr) => curr >= testmonyItems.length - 1 ? testmonyItems.length - 1 : curr + 1)} className='z-40 hidden md:block absolute top-[50%] right-4 cursor-pointer'>
            ...
            {/* current image  */}
            <div className='mx-4 md:mx-0 relative shadow-2xl md:absolute md:-top-5 md:left-[10%] w-[90%] md:w-[70%] grid grid-cols-[60%_40%] md:grid-cols-[40%_60%] gap-4 items-center  bg-white p-4 rounded-xl'>
                <div>
                ...
                <ArrowLeft className='w-14 h-14 md:w-20 md:h-20 text-black/30' />
            </div>)}
          { testmonyItems[imageIndex + 1]?.img && (  <div onClick={() => setImageIndex((curr) => curr >= testmonyItems.length - 1 ? testmonyItems.length - 1 : curr + 1)} className='md:hidden absolute top-[50%] right-4 cursor-pointer'>
                <ArrowRight className='w-14 h-14 md:w-20 md:h-20 text-black/30' />
            </div>)}
            </div>
        </div>
    </section>
