...
const PrescriptionCard: React.FC<PrescriptionProps> = ({
  aboutDrug,
  ageRange,
  concentration,
  dosage,
  ...
  const handleSatisfiedClient = (userId: string) => {
    ...
    if (sastifiedClient) {
      ...
      const prescriptionData: PrescriptionProps = {
        $id,
        productName,
        productImage,
        aboutDrug,
        ...
        whenTakeDosage,
        concentration,
        sastifiedClient: updatedClientNum,
      };
  ...
  return (
    <div className="bg-[#fbfcf8] p-4 rounded-lg my-4">
      ...
      <div className="p-4 rounded-lg flex items-center bg-white my-3 justify-between">
        ...
        />
        <CustomText
          text={String(sastifiedClient?.length ?? 0)}
          textType="normal"
          weightType="semibold"
        />
      </div>
      ...
      <div className="flex gap-2 items-center justify-between">
        {cart &&
        Array.isArray(cart) &&
        cart.find((p) => p.item.$id === productId)?.item.$id === productId ? (
          <></>
        ) : (
          <div
        ...
        wishlist.find((p) => p.$id === productId)?.$id === productId ? (
          <></>
        ) : (
          <div
      ...
      <div>
        ...
        <div className="grid grid-cols-3 my-3 gap-2">
          ...
          <DrugTab topText="Method Of Usage" bottomText={methodOfUsage} />
        </div>
      </div>
      {user &&
      user.userId &&
      sastifiedClient &&
      Array.isArray(sastifiedClient) &&
      sastifiedClient.includes(user && user.userId) ? (
        <></>
      ) : (
        <div className="flex gap-2 items-center">
          <CustomButton
