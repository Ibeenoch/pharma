...
const NavItems = () => {
    ...
    if (search.length >= 1) {
      ...
      if (searchType === "user") {
        let findUser = users.filter((u) => {
          return (
            u.firstName?.toLowerCase().startsWith(search.toLowerCase()) ||
  ...
  const openNotification = () => {
    ...
    const allUnreadNotification = notifications.filter(
      (n) => n.hasBeenRead === false
    );
    ...
    <>
      <div className={`flex items-center gap-5`}>
        <div className="relative flex -left-15 items-center">
          ...
          ${
            showSearchInput
              ? "opacity-100 scale-100 w-auto md:w-[350px]"
      ...
      <div
        ...
        } absolute top-14 w-7 rotate-[45deg] h-7 right-17 bg-white`}
      ></div>
      <div
        className={`${
          showNotification ? "block" : "hidden"
        } absolute bg-white right-[0%] top-15 w-[100%] md:w-[400px] h-screen overflow-y-auto rounded-md p-4`}
      >
        {/* Notification header  */}
          ...
          notifications.map(
            (n) =>
              ...
              n.$createdAt && (
                <div
                  key={n.$id}
                  className="grid grid-cols-[8%_72%_20%] items-center py-2 gap-2 border-b border-gray-200/90"
                >
      ...
      <div
        className={`${
          showSearchResult ? "block" : "hidden"
        } absolute bg-white right-0 md:right-[25%] top-14 w-[100%] md:w-[700px] rounded-md max-h-screen overflow-y-auto`}
      >
        <div
          ...
          {searchItems.map((s, i) => (
            <div onClick={() => setSearchText(s.toLowerCase(), i)}>
              <CustomText
                ...
                weightType="medium"
                color={
                  currentSearchTabIndex === i
                    ? "text-amber-500 border-b border-amber-500"
                    : "text-black"
                }
                extraStyle="text-center cursor-pointer"
              />
            </div>
          ))}
        </div>
        ...
        {searchType === "transaction" && (
          <>
            {foundTransaction &&
            Array.isArray(foundTransaction) &&
            foundTransaction.length > 0 ? (
